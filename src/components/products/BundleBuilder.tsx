            ...
            return (
              <div
                ...
                className={`relative rounded-2xl border p-5 transition-all duration-400 flex flex-col justify-between min-h-[240px] ${
                  hasProduct
                    ? "border-accent bg-surface ring-1 ring-primary/5 shadow-sm"
                    : "border-border-light bg-surface hover:border-muted hover:bg-background cursor-pointer hover:shadow-md"
                }`}
              >
                <div className="flex justify-between items-center mb-4">
                  <span className={`text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md bg-accent text-white`}>
                    Slot {index + 1}
                  </span>
                ...
                {slot.product ? (
                  <div className="flex gap-4 items-start flex-1">
                    <div className="relative w-16 h-20 rounded-xl overflow-hidden bg-surface border border-border-light flex-shrink-0">
                      {slot.product.image_url && (
                        <ProductImage src={slot.product.image_url} alt={slot.product.title} fill className="object-cover" sizes="64px" />
                      )}
                    </div>
                              ...
                              <button
                                ...
                                className={`text-[10px] font-bold px-2.5 py-1.5 rounded-lg border transition-all ${
                                  slot.size === sz
                                    ...
                                    : "bg-surface border-border-light hover:border-secondary"
                                }`}
                              >
                                {sz}
                              </button>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border border-dashed border-muted/30 rounded-xl bg-surface/50">
      ...
      <div className="border-t border-border-light bg-surface p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 shadow-inner">
        <div className="space-y-2 text-center md:text-left">
          ...
          <div className="flex items-center gap-3 justify-center md:justify-start flex-wrap">
            ...
            {isBundleComplete && (
              <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-md uppercase tracking-wider">
        ...
        <button
          ...
          className={`w-full md:w-auto min-w-[280px] h-16 flex items-center justify-center gap-3 rounded-2xl text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 shadow-xl ${
            isBundleComplete
              ? "bg-accent text-white hover:opacity-95 shadow-accent/10 hover:shadow-accent/20 scale-[1.02] cursor-pointer"
