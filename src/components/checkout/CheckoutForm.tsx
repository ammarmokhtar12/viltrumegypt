    ...
    <form className="space-y-10 font-sans" onSubmit={(e) => e.preventDefault()}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col rounded-xl border border-border-light overflow-hidden bg-background">
          {/* Vodafone Cash */}
          <label
            className={`flex items-center p-4 border-b border-border-light cursor-pointer transition-colors ${paymentMethod === "vodafone_cash" ? "bg-surface" : "hover:bg-surface"} `}
          >
          ...
          {/* InstaPay */}
          <label
            className={`flex flex-col p-4 cursor-pointer transition-colors ${paymentMethod === "instapay" ? "bg-surface" : "hover:bg-surface"}`}
          >
            <div className="flex items-center flex-1 gap-3">
              <input
                type="radio"
            ...
            {paymentMethod === "instapay" && (
              <div className="ml-7 mt-3 p-4 bg-background rounded-lg border border-border-light shadow-sm">
                <p className="text-xs text-secondary mb-2">
                  Please transfer exactly the total amount to:
                </p>
                <p className="text-lg font-bold tracking-wider text-foreground font-sans">
                  01031429229
                </p>
                <p className="text-[10px] font-medium text-muted mt-2">
                  You will be required to upload a screenshot proof after
                  checkout.
                </p>
              </div>
            )}
