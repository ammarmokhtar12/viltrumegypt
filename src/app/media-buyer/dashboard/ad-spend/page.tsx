  ...
  if (dbError) {
    return (
      <div className="max-w-lg mx-auto py-20 text-center space-y-4">
        ...
        <p className="text-xs text-zinc-500">Contact admin to set up the ad_spend table.</p>
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.3em] mb-1">Advertising</p>
          <h1 className="text-3xl font-bold text-white tracking-tight">Ad Spend</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-xs font-bold transition-all">
            <PlusCircle size={14} /> Add Entry
          </button>
          <button onClick={fetchAll} className="px-3 py-2.5 text-zinc-600 hover:text-white transition-colors border border-zinc-800 rounded-xl">
            <RefreshCw size={14} />
          </button>
        </div>
      </div>
        ...
        <form onSubmit={handleAdd} className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" step="0.01" required value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-white text-sm font-bold focus:outline-none focus:border-blue-500/50" />
            </div>
          </div>
